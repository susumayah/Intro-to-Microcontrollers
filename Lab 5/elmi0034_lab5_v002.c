...
#include "elmi0034_lab5_core_v002.h"
...
int main(void) {
    CLKDIVbits.RCDIV = 0; // Set clock divider to 1:1
    TRISBbits.TRISB5 = 0; // Set RB5 as output for LED (if needed for debugging)
    i2c_init();           // Initialize I2C module
    lcd_init();           // Initialize the LCD

    // Message to scroll
    const char *message = "Welcome Gophers!";
    const char *staticMessage = "EE 2361";
    while (1) {
        lcd_scrollMsg(message, staticMessage); // Scroll the message
    }
