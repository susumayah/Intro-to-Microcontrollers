...
CLKDIVbits.RCDIV = 0;
TRISBbits.TRISB5=0;
i2c_init(); // Initialize I2C before using it
...
while (1) {
int avgValue = getAvg();
float avgVoltage = (float)avgValue * (3.3 / 1024.0);
    char voltageStr[20];
sprintf(voltageStr, "%6.4f V", avgVoltage);
// Display the voltage on the LCD
lcd_cmd(0x01); // Clear display
lcd_printStr(voltageStr); // Print the voltage string
